...
public class EFCoreMigrationProvider : IDbMigrationProvider
{
    ...
    public async Task GenerateAsync(MigrationContext context, string migrationName)
    {
        ...
        try
        {
            ...
            // MAGIC TRICK: Make the migration dynamically schema-aware!
            if (!string.IsNullOrWhiteSpace(context.SchemaName))
            {
                ...
                foreach (var file in csFiles)
                {
                    ...
                    {
                        var replaced = text.Replace($"\"{context.SchemaName}\"", envReplacer);
    ...
    private async Task WriteDesignTimeFactoryAsync(MigrationContext context, string tempFactoryPath)
    {
        ...
        if (hasSchema)
        {
            // Build provider call with MigrationsHistoryTable inside the target schema
            ...
            // The subclass IS-A {contextFullName} so IDesignTimeDbContextFactory<T>
            // is still satisfied.
            factoryCode = $@"
...
{{
    // Schema-overriding wrapper generated by WonderDB Migration Tool.
    // Inherits the real DbContext so all entity mappings are preserved.
    ...
    {{
        ...
        {{
            // Apply target schema BEFORE base call so every entity table
            // and index lands in schema '{schemaName}'.
    ...
    {{
        ...
        {{
            ...
            // Return the schema-overriding subclass (it IS-A {contextFullName})
